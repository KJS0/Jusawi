...
def open_similar_search_dialog(viewer: "JusawiViewer") -> None:
    cur = getattr(viewer, "current_image_path", None)
    if not cur:
        from PyQt6.QtWidgets import QMessageBox  # type: ignore[import]
        QMessageBox.information(viewer, "유사 검색", "먼저 사진을 열어주세요.")
        return
    files = getattr(viewer, "image_files_in_dir", []) or []
    if not files:
        try:
    ...
    if not files:
        from PyQt6.QtWidgets import QMessageBox  # type: ignore[import]
        QMessageBox.information(viewer, "유사 검색", "현재 파일의 폴더를 확인할 수 없습니다.")
        return
    try:
        viewer._set_global_shortcuts_enabled(False)
    except Exception:
    ...
    try:
        ...
        d = SimilarSearchDialog(viewer, files=files, query_image=cur)
        d.resize(800, 600)
        try:
            d.setModal(False)
        except Exception:
        ...
        try:
            from PyQt6.QtWidgets import QMessageBox  # type: ignore[import]
            QMessageBox.warning(viewer, "유사 검색", f"다이얼로그 열기 실패: {e}")
        except Exception:
            pass
    finally:
        pass
